...
- **Conflict Resolution**: Uses CBS (Conflict-Based Search) as low-level individual path planning;  MGCBS as high-level multi-goal planning and conflict resolutions.
...
## Usage
> **Disclaimer**, the terms ***tasks per agents*** and ***goals per agent*** may used interchangeably.
...
The function will output results to the console and save detailed JSON and summary CSV files directly in the same directory, as for visualization it will saved in a directory like `na{num_agents}_nt{tasks_per_agent}_seed{seed}/`. You can check out [Output Demo](./Output%20Demo/) for a simple output example using 3 agents and 2 tasks per agent.
